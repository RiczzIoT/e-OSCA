...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_message'])) {
    ...
    $recipient_email = $recipient_data ? $recipient_data['email'] : '';

    if ($recipient_email) {
        $stmt = $conn->prepare("INSERT INTO messages (user_id, recipient_id, email, subject, body) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("iisss", $user_id, $recipient_id, $recipient_email, $subject, $body);
        $stmt->execute();
    }
...
<li class="nav-item dropdown">
    ...
    <div id="messages-dropdown-menu" class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
        <?php if ($message_count > 0): ?>
            ...
            <?php endforeach; ?>
            <a href=".inbox.php" class="dropdown-item dropdown-footer">See All Messages</a>
        <?php else: ?>
