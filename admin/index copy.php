...
document.addEventListener("DOMContentLoaded", function() {
    function updateRelativeTime() {
        ...
        elements.forEach(function(element) {
            ...
            var diff = Math.floor((now - timestamp) / 1000); // difference in seconds

            var relativeTime = '';

            if (diff < 60) {
                relativeTime = diff + ' secs ago';
            } else if (diff < 3600) {
                relativeTime = Math.floor(diff / 60) + ' mins ago';
            } else if (diff < 86400) {
                relativeTime = Math.floor(diff / 3600) + ' hours ago';
            } else if (diff < 604800) {
                relativeTime = Math.floor(diff / 86400) + ' days ago';
            } else {
                relativeTime = Math.floor(diff / 604800) + ' weeks ago';
            }
