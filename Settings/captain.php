...
if (!isset($_SESSION['user_id']) || !in_array($_SESSION['role'], ['admin', 'mayor', 'staff'])) {
...
if ($page_id === null || !is_numeric($page_id) || $page_id < 1) {
    ...
    include('../404/'); // Assuming you have a 404.php file for error page
    exit();
}
...
// Allow only admin to update favicon and logo
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($role === 'admin') {
        $favicon = isset($_POST['favicon']) ? $_POST['favicon'] : $favicon;
        $logo = isset($_POST['logo']) ? $_POST['logo'] : $logo;

        // Handle file uploads
        if (isset($_FILES['favicon']['name']) && $_FILES['favicon']['name']) {
            $favicon = '../assets/' . basename($_FILES['favicon']['name']);
            move_uploaded_file($_FILES['favicon']['tmp_name'], $favicon);
        }

        if (isset($_FILES['logo']['name']) && $_FILES['logo']['name']) {
            $logo = '../assets/' . basename($_FILES['logo']['name']);
            move_uploaded_file($_FILES['logo']['tmp_name'], $logo);
        }

        // Update the settings in the database
        $stmt = $conn->prepare("UPDATE settings SET favicon = ?, logo = ?, security_settings = ? WHERE user_id = ?");
        $stmt->bind_param("ssii", $favicon, $logo, $security_settings, $user_id);
        if ($stmt->execute()) {
        ...
        echo "You do not have permission to update these settings.";
    }
}
          ...
          <div class="card-body">
            <form action="update_image.php?id=249375180920" method="post" enctype="multipart/form-data">
              <div class="form-group">
                ...
                <?php if ($favicon): ?>
                  <p>Current Favicon: <a href="<?= htmlspecialchars($favicon) ?>" target="_blank">View</a></p>
                <?php endif; ?>
              </div>
              <div class="form-group">
                ...
                <?php if ($logo): ?>
                  <p>Current Logo: <a href="<?= htmlspecialchars($logo) ?>" target="_blank">View</a></p>
                <?php endif; ?>
              </div>
